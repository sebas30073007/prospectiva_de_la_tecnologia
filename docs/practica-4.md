---
title: Práctica 4 — Chatbot híbrido con APIs externas
layout: default
nav_order: 5
---

# Práctica 4: Chatbot híbrido con Ollama, Gemini y Groq
...
En esta práctica se desarrolló un **chatbot híbrido** capaz de conectarse con diferentes proveedores de modelos de lenguaje. El objetivo principal fue comparar el comportamiento de un modelo ejecutado de forma local mediante **Ollama** contra modelos remotos consumidos mediante **APIs externas**, específicamente **Gemini API** y **Groq API**.

El sistema permite seleccionar el proveedor, el modelo, el perfil del copiloto, el `system prompt` y parámetros de generación como `temperature`, `top_p` y número máximo de tokens. A partir de esto se puede analizar cómo cambia la respuesta del chatbot dependiendo del modelo y del proveedor utilizado.

---

## Objetivos

- Implementar un chatbot híbrido con distintos proveedores de modelos de lenguaje.
- Conectar un modelo local mediante Ollama.
- Conectar modelos remotos usando Gemini API y Groq API.
- Comparar tiempo de respuesta, tokens generados y velocidad de generación.
- Evaluar la calidad de las respuestas generadas.
- Analizar ventajas y desventajas de usar modelos locales frente a APIs externas.
...
La interfaz del chatbot contiene una sección de configuración y una sección de conversación. En la configuración se puede elegir el proveedor, el modelo, el perfil del copiloto y los parámetros de generación.
...
El sistema permite seleccionar entre tres proveedores principales:

- **Ollama local**
- **Gemini API**
- **Groq API**

Cada proveedor tiene asociado uno o más modelos disponibles. Para esta práctica se utilizaron principalmente los siguientes modelos:

| Proveedor | Modelo utilizado | Tipo |
|----------|------------------|------|
| Ollama local | `llama3.2:3b` | Local |
| Gemini API | `gemini-2.5-flash` | Remoto |
| Groq API | `llama-3.3-70b-versatile` | Remoto |
...
La interfaz también permite seleccionar distintos perfiles de copiloto. Cada perfil modifica el `system prompt`, que es la instrucción inicial que define el comportamiento del modelo.

### Perfil genérico

Este perfil configura al modelo como un asistente académico claro, preciso y útil.

![Perfil genérico](../assets/images/practica-4/perfil_generico.jpeg)

### Perfil de programación Python

Este perfil está enfocado en ayudar con código Python, explicación de errores y generación de soluciones paso a paso.

![Perfil de programación Python](../assets/images/practica-4/perfil_programacion_python.jpeg)

### Perfil de robótica móvil

Este perfil se utilizó para la comparación principal, ya que la prueba consistió en explicar un concepto relacionado con robots móviles.
...
Para comparar los modelos se utilizó el mismo prompt en los tres proveedores:
...
En la primera prueba se utilizó **Gemini API** con el modelo `gemini-2.5-flash`. La respuesta fue rápida, aunque en la captura se observa que la salida generada quedó corta en comparación con las otras pruebas.

![Respuesta generada con Gemini API](../assets/images/practica-4/RespuestaGemini.jpeg)

### Métricas obtenidas con Gemini
...
En la segunda prueba se utilizó **Groq API** con el modelo `llama-3.3-70b-versatile`. La respuesta fue más extensa y estructurada, incluyendo explicación conceptual, ecuaciones, ejemplo y limitación práctica.

![Respuesta generada con Groq API](../assets/images/practica-4/RespuestaGroq.jpeg)

### Métricas obtenidas con Groq
...
En la tercera prueba se utilizó **Ollama local** con el modelo `llama3.2:3b`. Esta prueba no dependió de una API externa, ya que el modelo se ejecutó directamente en la computadora. Sin embargo, el tiempo de respuesta fue mayor que en los proveedores remotos.

![Respuesta generada con Ollama local](../assets/images/practica-4/RespuestaOllama.jpeg)

### Métricas obtenidas con Ollama
...
A continuación se muestra el código usado para desarrollar el chatbot híbrido. Se incluyen los archivos principales del sistema: backend, frontend, estilos y dependencias.
...
Por otro lado, **Gemini API** y **Groq API** permiten acceder a modelos remotos sin necesidad de instalarlos localmente. Esto reduce los requisitos de hardware y facilita la integración, pero requiere conexión a internet, una API key y depender de las condiciones del proveedor.
...
Si se busca privacidad, control y ejecución local, **Ollama** es una alternativa adecuada. Si se busca facilidad de despliegue, modelos más grandes y menor carga computacional local, las **APIs externas** como Gemini o Groq pueden ser más convenientes.
